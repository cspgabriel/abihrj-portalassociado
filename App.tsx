...
import VanguardDashboard from './components/VanguardDashboard';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (params.get('vanguard') === 'true') {
        setCurrentView('VANGUARD_DASHBOARD');
    }
  }, []);
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full animate-fade-in-up">
           ...
           <div className="mt-6 text-center space-y-4">
             ...
             {!isForgotPassword && (
               <button
                 type="button"
                 onClick={() => { setIsPublicMode(true); setCurrentView('LANDING_PAGE'); setAuthError(''); setAuthSuccess(''); }}
                 className="text-gray-500 hover:text-rio-blue hover:underline text-xs flex items-center justify-center gap-1 mx-auto transition-colors"
               >
                 Ver portal sem login
               </button>
             )}
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'VANGUARD_DASHBOARD': return <VanguardDashboard user={user} onUseBenefit={handleBenefitClick} onViewDetails={(b) => { setSelectedBenefit(b); navigateTo('BENEFIT_DETAILS'); }} onNavigate={navigateTo} />;
      default: return <LandingPage userName={user.name} onNavigate={navigateTo} onBenefitClick={handleBenefitClick} />;

    }
  };

  const appUser: User = user || { id: 'guest', name: 'Visitante', email: '', hotel: '', role: '' };

  return (
    <Layout user={appUser} onLogout={handleLogout} onNavigate={navigateTo} onSearch={handleGlobalSearch} onBenefitClick={handleBenefitClick} currentView={currentView} isFullPage={['COURSES_V2', 'BENEFIT_CATEGORIZER', 'COMMERCIAL_ACTIONS_PAGE', 'WELCOME', 'PHOTO_GALLERY', 'TALENT_BANK', 'PUBLIC_ORDER_PAGE', 'LEGAL_ADVISORY_PAGE', 'MARKETING_KIT', 'BENEFITS_SHOWCASE', 'VANGUARD_DASHBOARD'].includes(currentView)}>
       {renderContent()}
